...
const ProductPage = () => {
  ...
  const extraArr = extra ? Object.entries(extra) : [];
  ...
  const featureListOne = extraArr?.map((item, i) => {
    return (
      <div key={i}>
        <span>{item.name} : </span> <span>{item.feature}</span>
      </div>
    );
  });

  const featuresArr = [];
  const specificationArr: Array<{ item: string; feature: string }> = [];
  const maxItems = 5;

  extraArr?.map((item, i) => {
    if (item.name && item.feature) {
      featuresArr.push({ item: item.name, feature: item.feature });

      specificationArr.push({ name: item.name, feature: item.feature });
    }
  });

  specArr?.map((feature, i) => {
    if (
      featuresArr.length < maxItems &&
      feature[0] !== "model" &&
      feature[0] !== "os"
    ) {
      featuresArr.push({ name: feature[0], feature: feature[1] });
    }
    if (
      feature[1] !== undefined &&
      feature[1] !== "" &&
      ...
      specificationArr.push({ name: feature[0], feature: feature[1] });
    }
  });
  ...
  const specificationList = specificationArr?.map((item, i) => (
    <div key={i} className="py-2 flex gap-4 capitalize">
  ...
  const handleAddToCart = () => {
    ...
    const defaultVariant = product?.variant[0]?.title
  ...
  return (
    <Container className="p-2 md:p-4">
          ...
          <div className="flex flex-col flex-1 ">
            {!loading ? (
              <>
                ...
                <h1 className="text-2xl font-semibold py-2 mt-2">
                  {product?.title}
                </h1>
              </>
            ) : (
            ...
            {!loading ? (
              <>
                {discount && (
                  <div className="font-semibold text-orange-600 py-1">
                ...
                <div className="py-4 leading-7 ml-2">{featureList}</div>
                <div className="flex justify-start items-center gap-2">
                  <span className="">Color: </span>{" "}
                  <span className="">{isSelected?.title}</span>
                </div>
                <div className="flex flex-wrap items-center justify-start gap-2">
                  <div className="flex justify-start items-center gap-2 py-2 mb-2 flex-wrap">
                    {colors?.length > 0 &&
                      colors.map((color, i) => (
                        <ColorCheckbox
                          key={i}
                ...
                <div className="">
                  ...
                  <div className="flex justify-start gap-4 py-4 items-center">
                    <span
                      className="py-1 px-3 font-semibold hover:bg-neutral-100 cursor-pointer rounded-full border-2 border-neutral-300  select-none"
                      onClick={() => handleQty(qty - 1)}>
                      -
                    </span>
                    <span className="w-16 py-2 text-center font-semibold border-2 border-neutral-300 cursor-default  select-none">
                      {qty}
                    </span>
                    <span
                      className="py-1  px-3 font-semibold hover:bg-neutral-100 cursor-pointer rounded-full border-2 border-neutral-300 select-none"
      ...
      <div className="flex justify-between border border-b border-b-neutral-200  w-full border-neutral-400">
        {tabs.map((item, i) => (
          <div
            key={i}
      ...
      <div className=" border-2 border-t-0 w-full border-neutral-400 p-4 md:p-10">
        {tab === "desc" && (
        ...
        {tab === "reviews" && (
          <div className="">
            <div className="flex flex-wrap justify-between items-start pb-4 border-b-2 border-orange-200">
              ...
              <div className="flex-1 w-full p-2 pl-10 md:w-3/5">
                <h4 className="pb-2 mb-2 border-b text-neutral-400 font-semibold">
                  Add Review
                </h4>
                {user ? (
      ...
      {!loading && (
        <div className="py-4 px-2">
          <h2 className="text-lg font-semibold py-2">Related Products</h2>
          <div className="px-2 flex justify-start gap-4 flex-wrap ">
            {relatedProducts}
          </div>
        </div>
      )}
