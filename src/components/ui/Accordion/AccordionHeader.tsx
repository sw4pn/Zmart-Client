import { FC, useMemo, useEffect, useRef, useContext } from "react";
...
const AccordionHeader: FC<Props> = ({
  ...
  const { hash, toggle, items, alwaysOpen, isActive } =
    ...
    if (ref && ref.current) {
      ...
      const showAccordion = (e: Event) => {
        ...
        if (!alwaysOpen) {
          ...
          const buttons = button.parentNode?.querySelectorAll<HTMLElement>(
            `:scope > ${TagName}[aria-expanded='true']`
          );
          if (buttons) {
        ...
        const content = document.querySelector(
          `#${button.getAttribute("aria-controls")}`
        ) as HTMLDivElement | HTMLUListElement;
        toggleContent(content);
